...
data["Temperature(℃)"] = encoder.fit_transform(data["Temperature(℃)"])  # แปลงเป็นตัวเลข

# แยก features (X) และ labels (y)
y = data["Temperature(℃)"].values  # ใช้ Weather Condition เป็น target
X = data.drop(columns=["Temperature(℃)"]).values.astype(float)  # ลบ Weather Condition ออกจาก X
...
model_path = r"C:\project2\Water_Use\model\09.00_model\09.00_temp.pkl"
