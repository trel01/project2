...
# โหลดข้อมูล
data = pd.read_csv(r"C:\project2\Water_Use\WeterCSV\weather-09.00-late_morning.csv")
...
data["Wind Speed(mph)"] = encoder.fit_transform(data["Wind Speed(mph)"])  # แปลงเป็นตัวเลข

# แยก features (X) และ labels (y)
y = data["Wind Speed(mph)"].values  # ใช้ Weather Condition เป็น target
X = data.drop(columns=["Wind Speed(mph)"]).values.astype(float)  # ลบ Weather Condition ออกจาก X
...
model_path = r"C:\project2\Water_Use\model\09.00_model\09.00_wind_speed.pkl"
