...
# โหลดข้อมูล
data = pd.read_csv(r"C:\project2\Water_Use\WeterCSV\water-21.00-late_at_night.csv")
...
data["Wind Speed(mph)"] = encoder.fit_transform(data["Wind Speed(mph)"])

# แยก features (X) และ labels (y)
y = data["Wind Speed(mph)"].values  # Target
X = data.drop(columns=["Wind Speed(mph)"]).values.astype(float)  # Features
...
model_path = r"C:\project2\Water_Use\model\21.00_model\21.00_wind_speed.pkl"
